...
var db = new sqlite3.Database('redirectYannick.db');
var check;
var channels;
var users;
...
bot.on('start', function () {
  db.run("CREATE TABLE if not exists user_redirect (user TEXT, user_to TEXT, dest TEXT, date TEXT)");
  dbRead();
  //addRedirect();
...
function addRedirect(user, userTo, dest, date) {
  ...
  db.run("INSERT INTO user_redirect VALUES ('" + user + "','" + userTo + "','" + dest + "','"+ date +"')");
}
...
function apiTravel(data) {
  ...
  if (isPrivate(data.channel)) {
    ...
    try {
      ...
      bot.postMessageToUser(users.getByProps({id: data.user})[0].name, "The date is missing or has a bad format. Correct format is yyyy-mm-dd (ex:2016-06-04) ");
    }
    }
    catch
    (err) {
